...
class Pty {
  ...
  Pty.start(
    ...
    // Modern TUIs (including Claude Code) use COLORTERM in addition to TERM
    // when deciding whether to emit ANSI colors. Without it they can choose a
    // monochrome palette even though this PTY renders 24-bit colors.
    effectiveEnv['COLORTERM'] = 'truecolor';
    effectiveEnv['TERM_PROGRAM'] = 'VibeTerminal';
    ...
    for (var i = 0; i < effectiveEnv.length; i++) {
      ...
      envp.elementAt(i).value = pointer;
    }
    ...
    options.ref.executable = executablePointer.cast();
    ...
    if (workingDirectoryPointer != null) {
      options.ref.working_directory = workingDirectoryPointer.cast();
    } else {
    ...
    try {
      ...
      if (workingDirectoryPointer != null) {
        malloc.free(workingDirectoryPointer);
      }
      calloc.free(argv);
      calloc.free(envp);
      calloc.free(options);
