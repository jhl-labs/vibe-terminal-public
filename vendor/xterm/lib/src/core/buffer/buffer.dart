...
class Buffer {
  ...
  // 스크롤 영역 이동은 반드시 moveItem을 쓴다.
  //
  // `lines[i] = lines[i ± n]` 으로 대입하면 같은 BufferLine 객체가 두 인덱스에
  // 걸쳐 있다가, 원래 자리를 덮어쓰는 순간 방금 옮긴 라인이 순환 버퍼에서
  // 분리(detach)된다. 분리된 라인에는 CellAnchor가 붙지 못하고,
  // TerminalController.selection은 앵커가 detached면 null을 반환하므로
  // 드래그 선택이 통째로 동작하지 않게 된다. codex 같은 인라인 TUI는 DECSTBM으로
  // 스크롤 영역을 잡아 이 경로를 계속 타기 때문에 증상이 항상 재현된다.

  void scrollDown(int lines) {
    final top = absoluteMarginTop;
    final bottom = absoluteMarginBottom;
    for (var i = bottom; i >= top + lines; i--) {
      this.lines.moveItem(i - lines, i);
    }
    final lastCleared = bottom < top + lines - 1 ? bottom : top + lines - 1;
    for (var i = top; i <= lastCleared; i++) {
  ...
  void scrollUp(int lines) {
    ...
    final firstCleared = bottom - lines + 1 > top ? bottom - lines + 1 : top;
    for (var i = firstCleared; i <= bottom; i++) {
      this.lines[i] = _newEmptyLine();
    }
  }
  ...
  void index() {
    if (isInVerticalMargin) {
      if (_cursorY == _marginBottom) {
        if (marginTop == 0 && !isAltBuffer && _marginBottom == viewHeight - 1) {
  ...
  void resize(int oldWidth, int oldHeight, int newWidth, int newHeight) {
    ...
    if (newWidth != oldWidth) {
      if (terminal.reflowEnabled && !isAltBuffer) {
        // 커서를 논리 줄과 함께 옮긴다. (Vibe Terminal 패치, reflow.dart 참고)
        final tracked = reflowTracked(
          lines,
          oldWidth,
          newWidth,
          cursorY: _cursorY + (lines.length - newHeight),
          cursorX: _cursorX,
        );
        final reflowResult = tracked.lines;
        ...
        final newCursorY = tracked.cursorY;
        if (newCursorY != null) {
          // replaceWith 는 maxLength 를 넘는 앞부분을 버리므로 그만큼 보정한다.
          final dropped = reflowResult.length - lines.length;
          _cursorY = (newCursorY - dropped - (lines.length - newHeight)).clamp(
            0,
            newHeight - 1,
          );
          _cursorX = tracked.cursorX!.clamp(0, newWidth - 1);
        }
      } else {
