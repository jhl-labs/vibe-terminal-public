...
class EscapeParser {
  ...
  void _csiHandleSgr() {
    // Private CSI sequences such as `CSI > 4 m` are keyboard option controls,
    // not Select Graphic Rendition. Claude Code emits this while probing the
    // terminal. Treating it as plain `CSI 4 m` leaks underline onto every cell
    // subsequently drawn by the TUI.
    if (_csi.prefix != null) {
      return;
    }
