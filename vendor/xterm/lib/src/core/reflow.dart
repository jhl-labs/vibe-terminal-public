...
/// Result of [reflow]. [cursorY]/[cursorX] are the absolute row and column
/// where the tracked cursor ends up, or null if no cursor was tracked.
class ReflowResult {
  const ReflowResult(this.lines, {this.cursorY, this.cursorX});
...
List<BufferLine> reflow(
  ...
  return reflowTracked(lines, oldWidth, newWidth).lines;
}

/// Reflows [lines] from [oldWidth] to [newWidth]. When [cursorY] (absolute
/// row) and [cursorX] are given, the cursor is carried along with the logical
/// line it sits on so it keeps pointing at the same text after the reflow.
///
/// Vibe Terminal patch: the original never adjusted the cursor. Inline TUIs
/// (claude, codex) redraw relative to the cursor, so a width change that
/// re-wrapped lines above the cursor left the cursor row stale and every
/// subsequent redraw landed on the wrong rows until the screen was reset.
ReflowResult reflowTracked(
  IndexAwareCircularBuffer<BufferLine> lines,
  int oldWidth,
  int newWidth, {
  int? cursorY,
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    final groupEnd = i;
    final newGroupStart = result.length;
    final produced = reflow.finish();
    result.addAll(produced);

    if (cursorY != null && cursorY >= groupStart && cursorY <= groupEnd) {
      final offset = (cursorY - groupStart) * oldWidth + (cursorX ?? 0);
      final groupLength = produced.isEmpty ? 1 : produced.length;
      final row = (offset ~/ newWidth).clamp(0, groupLength - 1);
      newCursorY = newGroupStart + row;
      newCursorX = (offset - row * newWidth).clamp(0, newWidth - 1);
    }
  }
  ...
  return ReflowResult(result, cursorY: newCursorY, cursorX: newCursorX);
}
