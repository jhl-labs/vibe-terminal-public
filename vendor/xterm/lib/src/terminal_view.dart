...
class TerminalViewState extends State<TerminalView> {
  ...
  /// The scroll controller driving the scrollback viewport. Vibe Terminal
  /// patch: lets drag-selection auto-scroll move the viewport.
  ScrollController get scrollController => _scrollController;

  /// 현재 터미널 내용을 변경하지 않고 뷰포트 전체를 다시 그린다.
  void requestRepaint() {
    renderTerminal.requestRepaint();
  }

  bool _scrollbackFrozen = false;

  bool get _terminalFreezesScrollback =>
      widget.terminal.mouseMode.reportScroll ||
      widget.terminal.isUsingAltBuffer;

  void _onTerminalModeChanged() {
    final frozen = _terminalFreezesScrollback;
    if (frozen != _scrollbackFrozen) {
      _scrollbackFrozen = frozen;
      if (mounted) setState(() {});
    }
  }
  ...
  void initState() {
    ...
    _scrollbackFrozen = _terminalFreezesScrollback;
    widget.terminal.addListener(_onTerminalModeChanged);
    super.initState();
  }
  ...
  void didUpdateWidget(TerminalView oldWidget) {
    if (oldWidget.terminal != widget.terminal) {
      oldWidget.terminal.removeListener(_onTerminalModeChanged);
      widget.terminal.addListener(_onTerminalModeChanged);
      _scrollbackFrozen = _terminalFreezesScrollback;
    }
  ...
  @override
  void dispose() {
    widget.terminal.removeListener(_onTerminalModeChanged);
  ...
  Widget build(BuildContext context) {
    // Vibe Terminal patch: while the application receives wheel reports the
    // local scrollback must not consume the wheel first, otherwise scrolling
    // up moves the scrollback instead of reaching the TUI.
    final scrollbackFrozen = widget.terminal.mouseMode.reportScroll ||
        widget.terminal.isUsingAltBuffer;
    Widget child = Scrollable(
      key: _scrollableKey,
      controller: _scrollController,
      physics: scrollbackFrozen ? const NeverScrollableScrollPhysics() : null,
      viewportBuilder: (context, offset) {
        return _TerminalView(
          ...
          repaintBackgroundColor:
              widget.backgroundOpacity == 1 ? widget.theme.background : null,
...
class _TerminalView extends LeafRenderObjectWidget {
  const _TerminalView({
    ...
    required this.theme,
    required this.repaintBackgroundColor,
  ...
  final Color? repaintBackgroundColor;
  ...
  RenderTerminal createRenderObject(BuildContext context) {
    return RenderTerminal(
      ...
      theme: theme,
      repaintBackgroundColor: repaintBackgroundColor,
      focusNode: focusNode,
