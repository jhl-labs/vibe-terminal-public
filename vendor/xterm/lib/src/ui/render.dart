...
class RenderTerminal extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  RenderTerminal({
    ...
    required Color? repaintBackgroundColor,
    ...
    String? composingText,
        ...
        _repaintBackgroundColor = repaintBackgroundColor,
  ...
  /// 불투명 TerminalView가 repaint될 때 이전 프레임의 글자 장식과 커서
  /// 픽셀을 지우기 위해 먼저 칠할 배경색. 투명 배경에서는 null이다.
  Color? get repaintBackgroundColor => _repaintBackgroundColor;
  Color? _repaintBackgroundColor;
  set repaintBackgroundColor(Color? value) {
    if (value == _repaintBackgroundColor) return;
    _repaintBackgroundColor = value;
    markNeedsPaint();
  }
  ...
  void selectCharacters(Offset from, [Offset? to]) {
    ...
    if (to == null) {
      ...
      final (begin, end) = _inclusiveSelectionRange(fromPosition, toPosition);
      _controller.setSelection(
  ...
  /// Selects characters from the buffer cell [from] to the cell under the
  /// pixel offset [to]. Vibe Terminal patch: a drag keeps its anchor as a cell
  /// so that scrolling mid-drag (wheel or auto-scroll) does not move the
  /// anchor along with the viewport.
  void selectCharactersFromCell(CellOffset from, Offset to) {
    final toPosition = getCellOffset(to);
    final (begin, end) = _inclusiveSelectionRange(from, toPosition);
    _controller.setSelection(
      _terminal.buffer.createAnchorFromOffset(begin),
      _terminal.buffer.createAnchorFromOffset(end),
    );
  ...
  (CellOffset, CellOffset) _inclusiveSelectionRange(
    CellOffset from,
    CellOffset to,
  ) {
    final fromFirst = from.isBefore(to) || from.isEqual(to);
    final begin = fromFirst ? from : to;
    final lastIncluded = fromFirst ? to : from;
    return (begin, CellOffset(lastIncluded.x + 1, lastIncluded.y));
  }
  ...
  /// 터미널 버퍼를 변경하지 않고 repaint boundary를 다시 그린다.
  void requestRepaint() {
    markNeedsPaint();
  }
  ...
  void _paint(PaintingContext context, Offset offset) {
    ...
    final repaintBackgroundColor = _repaintBackgroundColor;
    if (repaintBackgroundColor != null) {
      // A normal opaque drawRect can still be folded into a retained GPU layer
      // on some desktop renderers.  Claude-style inline TUIs repaint the same
      // rows very quickly, which can then leave text-decoration pixels from an
      // older frame behind.  Source replacement makes this an explicit clear
      // of the repaint boundary while the clip keeps it inside the viewport.
      canvas.save();
      canvas.clipRect(offset & size);
      canvas.drawRect(
        offset & size,
        Paint()
          ..color = repaintBackgroundColor
