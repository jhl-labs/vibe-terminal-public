...
class IndexAwareCircularBuffer<T extends IndexedItem> {
  ...
  /// Moves the item at [fromIndex] to [toIndex], leaving [fromIndex] empty.
  ///
  /// Prefer this over `list[to] = list[from]` when relocating items that are
  /// already in the buffer. Assignment adopts the item at the destination while
  /// the source slot keeps pointing at the same object, so overwriting the
  /// source later detaches the item that was just relocated. A detached line
  /// cannot host [CellAnchor]s, which silently breaks text selection.
  void moveItem(int fromIndex, int toIndex) {
    RangeError.checkValueInInterval(fromIndex, 0, length - 1, 'fromIndex');
    RangeError.checkValueInInterval(toIndex, 0, length - 1, 'toIndex');
